...
>(["specs"]); 
    ...
    if (!product) {
        return (
            <main className="min-h-screen pt-[var(--nav-h)] bg-black text-black grid place-items-center px-6">
                        ...
                        {related.map((p) => (
                            ...
                            >
                                ...
                                <div className="px-5 pb-5">
                                    <div className="text-black/75 text-sm">{p.brand}</div>
                                    <div className="mt-1 text-black font-semibold">{p.name}</div>
                                    <div className="mt-1 text-black/55 text-sm">{p.code}</div>
                                </div>
                            </button>
                        ))}
                    </div>
  ...
  >
    ...
    >
      <button
        type="button"
        onClick={() => setDimPreview(null)}
        className="absolute top-4 right-4 h-10 w-10 rounded-full border border-white/15 bg-black/40 backdrop-blur grid place-items-center text-black/85 hover:text-black hover:border-white/30 transition duration-300 ease-in-out"
